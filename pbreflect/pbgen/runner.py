...
def run(
    ...
    directory_patcher = DirectoryStructurePatcher(output_dir)
    directory_patcher.patch()

    import_patcher = ImportPatcher(output_dir, root_path)
    import_patcher.patch()

    if gen_type == "mypy":
        mypy_patcher = MypyPatcher(output_dir, root_path)
        mypy_patcher.patch()
